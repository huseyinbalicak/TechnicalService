...
import com.balicak.TeknikServis.model.Booking;
import com.balicak.TeknikServis.model.BookingDTO;
import com.balicak.TeknikServis.model.Users;
import com.balicak.TeknikServis.repo.BookingRepo;
import com.balicak.TeknikServis.service.BookingService;
import com.balicak.TeknikServis.service.UserService;
...
public class BookingController {
    ...
    public BookingController(BookingService bookingService, BookingRepo bookingRepo, UserService userService, MessageSource messageSource, BookingValidator bookingValidator) {
    ...
    public ResponseEntity<BookingDTO> getByIdQueryParam(@RequestParam(value = "id", required = true) Integer id) {
        ...
        BookingDTO res = bookingService.getById(id);
        if (res != null) {
    ...
    public ResponseEntity save(@Validated @RequestBody Booking booking, BindingResult result, @RequestHeader(name="Accept-Language", required = false) Locale locale)
    {
        ...
        {
            return ResponseEntity.status(HttpStatus.CREATED).body(messageSource.getMessage("booking.save.success",null, locale)+ bookingRepo.getNextId());
        }
        else
        {
    ...
    public ResponseEntity<String> deleteById(Authentication auth,@Parameter (name = "Booking ID", required = true) @PathVariable(value = "id") Integer id,@RequestHeader(name = "Accept-Language", required = false) Locale locale) {
        ...
        if (!usr.getUSER_ID().equals(bookingRepo.getBookingById(id).getUSER_ID()))
        {
    ...
    public ResponseEntity getAppointmentsDatesInOrder(@RequestParam(name = "sortType") String type,@RequestHeader(name = "Accept-Language", required = false) Locale locale){
        ...
        return ResponseEntity.ok(RestResponse.of(bookingService.getAppointmentsDatesInOrder(type, locale)));
    }
    ...
    public ResponseEntity getAllAppointmentLikeUsername (@RequestParam(name = "username") String username,@RequestHeader(name = "Accept-Language", required = false) Locale locale){
        ...
        return ResponseEntity.ok(RestResponse.of(bookingService.getAllAppointmentLikeUsername(username,locale)));
    }
    ...
    public ResponseEntity<?> appointmentIsComplete(@RequestParam(name = "id") int id, @RequestParam (name = "is_done") boolean is_done,@RequestHeader(name = "Accept-Language", required = false) Locale locale){
        ...
        return ResponseEntity.ok(RestResponse.of(bookingService.appointmentIsComplete(id,is_done, locale)));    }
